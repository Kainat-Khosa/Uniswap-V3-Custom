...
describe("LiquidityEamples", () => {
    let liquidityEamples;
    let accounts;
    let dai;
    let usdc;
    ...
    before(async () => {
        ...
        const LiquidityEamples = await ethers.getContractFactory("LiquidityExamples");
        liquidityEamples = await LiquidityEamples.deploy();
        await liquidityEamples.deployed();
    ...
    it("mintNewPosition", async () => {
        ...
        await dai.connect(accounts[0]).transfer(liquidityEamples.address, daiAmount);
        await usdc.connect(accounts[0]).transfer(liquidityEamples.address, usdcAmount);

        await liquidityEamples.mintNewPosition();
