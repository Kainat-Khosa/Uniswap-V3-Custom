...
async function main() {
  ...
  const usdtContract = new Contract(TETHER_ADDRESS, artifacts.Usdt.abi, defaultSigner);
  const usdcContract = new Contract(USDC_ADDRESS, artifacts.Usdc.abi, defaultSigner);

  await usdtContract.approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('1000'));
  await usdcContract.approve(POSITION_MANAGER_ADDRESS, ethers.utils.parseEther('1000'));

  const poolContract = new Contract(USDT_USDC_500, artifacts.UniswapV3Pool.abi, defaultSigner);
  ...
  const nonfungiblePositionManager = new Contract(
    ...
    defaultSigner // Use default signer
  );
